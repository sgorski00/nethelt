...
/**
 * Manages scheduled tasks for network operations based on configurations fetched from a web
 * service.
 */
public class WebScheduledTaskManager implements ScheduledTaskManager {
  ...
  /**
   * Starts the scheduled task manager, which periodically fetches network configurations and
   * updates the scheduled tasks accordingly.
   */
  public void start() {
    LOG.info("Starting Scheduled Task Manager");
    scheduler.scheduleWithFixedDelay(
        this::updateTasks, 0, CONFIG_UPDATE_INTERVAL_SEC, TimeUnit.SECONDS);
  }
  ...
  private ScheduledFuture<?> rescheduleTask(
      ScheduledFuture<?> currentTask, int intervalSec, Runnable task) {
